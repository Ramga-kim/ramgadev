...
- `receipt_worker`는 이 스킬을 다시 로드하거나 전체 경비청구 workflow를 재실행하지 않는다. 자신의 역할은 영수증 추출과 첨부 산출물 준비로 제한한다.
- `receipt_worker`는 멀티모달 LLM으로 이미지/PDF 내용을 읽어 추출한다. Python, Node.js, ImageMagick, Ghostscript, 커스텀 OCR 스크립트 탐색을 시도하지 않는다.
- vendor는 원문에 보이는 상호명을 보수적으로 추출하며, `본점`을 `분점`으로 바꾸는 식의 추정이나 의역을 하지 않는다.
- 중복 판정은 승인번호 우선, 없으면 날짜+금액+상호 조합으로 수행한다.
- 최종 반환의 `json:` 값은 반드시 전체 JSON 배열이어야 하며, 객체 나열이나 느슨한 포맷을 사용하지 않는다.
