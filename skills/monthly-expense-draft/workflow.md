...
- `zip`, `7z`, `rar`, `pdf`, `xlsx` 등 비이미지 파일은 참고하지 말고 압축 해제도 시도하지 않는다.
...
- 권장 순서:
  1. 브라우저 자동화 가능 여부 확인
  2. 브라우저 워커가 Goworks 폼 접속 및 로그인 상태 확인
  3. 메인 워커가 영수증 루트 경로를 확인하거나 사용자에게 질문
  4. 루트 경로가 정해지는 즉시 파일 처리와 OCR 시작
  5. OCR 결과가 준비되면 이미 로드된 폼에 일괄 입력
