...
2. 예시 경로: `C:\Users\<USER>\Desktop\영수증` 또는 `바탕화면\영수증`
3. 설정값이 비어 있으면 질문받은 값을 현재 실행에 즉시 반영한 뒤 계속 진행한다.
4. `skill-config.yaml`이 있으면 `business.receipt_root`에 방금 받은 경로를 저장해 다음 실행에서도 재사용한다.
5. `skill-config.yaml`이 없고 로컬 설정 저장이 가능하면 생성하거나 업데이트해서 같은 경로를 기억한다.
6. 저장이 불가능한 환경이면 최소한 현재 실행 동안 메모리에 유지하고 다시 묻지 않는다.
7. 루트 하위 폴더 구조를 탐색해서 연도별 폴더 여부와 월별 네이밍 패턴을 파악한다.
8. 컨벤션이 불명확한 경우에만 추가 질문한다.
9. 그 외 단계는 자동 수행한다.
10. 단, 브라우저 워커는 이 질문의 답을 기다리지 말고 먼저 Goworks 기안 페이지 접속, 로딩 대기, 로그인 상태 확인까지 병렬로 진행한다.
...
- 사용자에게 영수증 루트 경로를 묻는 동안에도 브라우저 워커는 idle 상태로 대기하지 말고, 루트 경로 없이 가능한 폼 준비 작업을 먼저 끝낸다.
